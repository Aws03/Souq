...
| # | Blocker | Evidence | Remediation | Tests | Status |
|---|---|---|---|---|---|
| R-08 | **A suspended store answers 503 to everything**, including its own storefront configuration and sign-in, so it cannot show a branded "closed" page and its administrators cannot sign in to fix anything. `AvailableWhenStoreClosedAttribute` exists and is honoured — and is applied to **no endpoint** | Verified in code: a repository-wide search finds the attribute only in its own definition, in `TenantAvailabilityMiddleware.IsOpen`, and in the documentation generator | Apply it to the storefront configuration endpoint and to sign-in, keeping everything else closed. How much administration a *suspended* store should retain is a product decision recorded below | `TenantResolutionTests`, `AuthorizationBoundaryTests` | **OPEN — in progress (Phase 17)** |
...
| R-10 | **The public order tracking token is written to the request log.** `RequestLoggingMiddleware` logs `Request.Path`, and the token is a path segment of `/api/orders/track/{token}`; nginx's default access log records it too | Verified in code | Redact sensitive route parameters from the logged path while keeping the route template for aggregation; decide the proxy's log policy | `ObservabilityTests`, `NotificationTests` | **OPEN — in progress (Phase 17)** |
