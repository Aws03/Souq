...
### 12 · Roadmap
[ProductRoadmap](12-ROADMAP/ProductRoadmap.md) (phases, status, the decision log) · [TechnicalDebt](12-ROADMAP/TechnicalDebt.md) · [CommercialReadiness](12-ROADMAP/CommercialReadiness.md) (what it takes to *sell* this: confirmed multi-tenant capabilities, how a store is provisioned today, what a merchant can customize, the commercial gaps and the order to close them) · [CommercialPlatformArchitecture](12-ROADMAP/CommercialPlatformArchitecture.md) (the durable design for the commercial SaaS layer — the control plane, the two money paths, the provider-agnostic payment port, quotas, events, customization and scale-out, subsystem by subsystem) · [CommercialPlatformPlan](12-ROADMAP/CommercialPlatformPlan.md) (its phases `C1`–`C14`, their dependencies, what is deliberately deferred, and the decisions that are the owner's) · [OwnerDecisionBrief](12-ROADMAP/OwnerDecisionBrief.md) (the seven blocking decisions written for the owner rather than for an engineer: what each one is in plain language, the realistic options, what each costs, what is already built, and one precise question per decision) · [SouqMasterPlan](12-ROADMAP/SouqMasterPlan.md) (the durable execution contract for the twenty engineering-mission phases — `M1`–`M20` — that carry the product from here to launch readiness; a fresh Claude session recovers exactly where things stand from its §0 status block alone)
