...
| A variant is one complete, unique combination of its product's option values; a value any variant uses is never removed; the default variant is always active; every option or variant change is concurrency-guarded on the product row | A duplicate or incomplete combination can't be chosen, labelled or stocked truthfully, and variants are referenced forever | `Product.SetOptions`, `Product.AddVariant`, unique `(ProductId, CombinationKey)`, restrict FK to `ProductOptionValues`, `IProductRepository.GuardConcurrentEdit`; `ProductOptionTests`, `ProductOptionAdminTests` | [ADR-0040](../11-ADR/0040-product-option-model.md) |
| The storefront never shows a price that cannot be paid, and never offers a variant a shopper cannot buy: the displayed price is the cheapest purchasable variant, and a deactivated variant is absent from the read model | A card, a filter and a sort must agree, and an advertised price must be payable (P-08b) | `CatalogQueries` (one SQL expression for display, filter and sort), `WishlistQueries`; `StorefrontVariantTests` | [ADR-0041](../11-ADR/0041-storefront-variant-selection.md) |
