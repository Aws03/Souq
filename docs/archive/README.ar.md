> 📌 هذا هو الدليل التعليمي الأصلي للمشروع (عربي). الـ README الرئيسي على
> GitHub الآن بالإنجليزي في جذر المستودع: [`/README.md`](../../README.md).
...
### 4) بديل: تشغيل الحزمة كاملة عبر Docker
حزمة معزولة تماماً (قاعدة بياناتها الخاصة، لا تلمس حاوية `sqlserver` المحلية
إن وُجدت): انسخ `.env.example` إلى `.env` واملأ القيم، ثم:
```bash
docker compose up --build
```
الواجهة على `http://localhost:8081`، والـ API مباشرة على `http://localhost:5201`
(للتشخيص). الهجرات والبذر يعملان تلقائياً عند الإقلاع، كما في التشغيل المحلي.
...
`Payments:Provider=Demo` صراحةً (عرض توضيحي فقط). لاختبار فشل الدفع فعلياً فعِّل
