...
2. **Done in Phase 15** ([ADR-0035](../11-ADR/0035-white-label-runtime.md)): brand references and the hard-coded currency were removed from the frontend, `frontend/index.html` and the committed backend configuration; tenant branding replaced the visitor theme switcher; the four old palettes are no longer offered to visitors. Selectable colour presets belong in a branding editor (**PLANNED**, Phases 17–18); until then a store's colours are set through the settings API.
