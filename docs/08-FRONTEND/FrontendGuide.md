...
**The cyclic focus trap is implemented** (TD-48, closed 2026-09-21). It lives in `useDialog`, so every drawer and dialog has it, and `ProductZoom` uses the hook rather than its own copy of the same behaviours. Tab and Shift+Tab cycle at the boundaries; in the middle the browser's own tab order is left alone, because it is more accurate than any order recomputed from a selector. The list of focusable elements is read on each keypress rather than cached when the dialog opens, since content changes as tabs switch and fields appear. Escape still closes — a dialog should confine focus, not trap the person in it. Visibility is decided with `checkVisibility()` rather than `offsetParent`, which is null for any `position: fixed` element and would have disabled the trap on exactly the dialogs it guards; where that API is unavailable the element is included, because an extra tab stop is a much better failure than an open page behind a modal.
