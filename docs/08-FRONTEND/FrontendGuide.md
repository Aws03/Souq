...
**Why there is no global store and no query library.** D-19 (TypeScript and TanStack Query) is **DEFERRED** with a trigger ([ADR-0035](../11-ADR/0035-white-label-runtime.md)): both are large dependency and toolchain changes that need approval, Phase 15 did not need them, and the storefront and admin rebuilds (Phases 16–17) rewrite most data-fetching screens anyway — introducing the query layer there avoids writing those screens twice. The trigger is the start of Phase 16 or the addition of a CI type-check.

What that costs today: no caching, no request de-duplication and no retry policy. Every screen refetches on mount, the category list is fetched once per storefront layout and shared through the outlet context, and effects that can be outrun by a fast navigation guard themselves with a local `active` flag (`useCatalog`, `Store`, `TenantProvider`, `WishlistProvider` do; `ProductDetail` does not).
...
**Stripe.** `frontend/src/pages/checkout/stripeClient.js` loads Stripe.js once per session with the publishable key from GET `/api/payments/config`. An empty key means the store has no gateway configured, and `CardPaymentForm` silently falls back to a single confirm button that goes through the server's fake gateway — the store stays usable without Stripe keys. Card details never reach the Souq API.
...
**Not covered today:** there are no component, hook, context or router tests — no rendering library is installed, so guards, layouts, forms and the providers are only exercised by hand. `frontend/src/app/storeTheme.js` (document side effects), `frontend/src/features/account/download.js`, `frontend/src/pages/checkout/stripeClient.js` and every page are untested. There is no end-to-end suite and no CI pipeline running any of this. Frontend unit tests and an E2E suite are **PLANNED** for Phase 19.
...
| 3 | `frontend/src/api/client.js` is one module for the whole product | 92 endpoint functions covering every feature | Every screen imports the whole surface; splitting is tied to D-19 |
...
| 10 | No component or route tests | No rendering library in `frontend/package.json`; all 23 test files cover pure modules | Guards, layouts, providers and forms are verified only by hand |
...
| 12 | Unguarded async effects | `ProductDetail` sets state from three requests without an `active` flag, unlike `useCatalog` and `Store` | Fast navigation between products can render the previous product's data |
...
| 15 | Stripe availability is cached, including failure | `frontend/src/pages/checkout/stripeClient.js` stores the promise for the session; a transient failure of `/api/payments/config` resolves to `null` | The visitor is offered the fallback confirm button for the rest of the session |
| 16 | Permission and module strings are untied literals | `frontend/src/App.jsx` and `frontend/src/pages/admin/AdminSidebar.jsx` repeat the server's permission names | A renamed permission fails silently as a hidden or a rejected page |
| 17 | A store in provisioning cannot open the SPA | GET `/api/storefront/config` is not marked available while a store is closed, so a `Provisioning` store answers `503 StoreUnavailable` and the SPA shows the "closed" screen — although `AuthController` is marked `AvailableDuringProvisioning` precisely so its administrators can sign in and prepare it | The admin UI cannot be used to prepare a store before opening; relevant to Phases 17–18 |
...
- **Splitting `frontend/src/api/client.js` per feature waits for the query layer**, because the query layer decides what those modules look like (D-19, DEFERRED).
