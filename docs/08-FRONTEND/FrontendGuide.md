...
- **`npm run build`** produces the Vite bundle: one eager chunk plus a lazy chunk per page, and one chunk per locale. Measured on the production build served by `vite preview`, gzipped, over every JS, CSS and HTML response of a cold first load: **136.5 kB in Arabic and 134.2 kB in English**. The locale files are dynamic imports, so a visitor downloads one language rather than both — that alone was about 14 kB gzipped. No budget is enforced; bundle-size budgets are **PLANNED** for Phase 21.
