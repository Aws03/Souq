...
| Browser | `frontend/e2e/checkout-reliability.spec.js` (M5) | the adversarial half, on a real stack: a line that sells out between page load and submit is refused **with a reason on screen** rather than a dead button; a double click sends exactly one `POST /api/orders` (the submit button disables while busy, so the browser is not a source of F-8 duplicates); and the happy path still reaches a paid order after the handler split |
