...
- **Delivered (the reporting slice of roadmap Phase 18):** the platform overview is the first real consumer of `GET /api/platform/stats`. Per-store "health" was delivered by Phase 18 as handover readiness on `/platform/stores` (a domain, an administrator who accepted) — a [Platform](../Platform/README.md) list, not a report. **That last sentence used to read "no store's commercial figures reach the platform by design", and C11 reverses it deliberately.** It was right for a platform that sells nothing and impossible for one that **bills** its merchants: a commission cannot be computed without knowing what was sold, and owner decision `C-03` (the commission basis) presumes this figure exists. `GET /api/platform/revenue` now returns store revenue for a period, behind `platform.reports.view`, audited like every platform request, and read through `PlatformQueries` — still the one reviewed type allowed to bypass the tenant filter. Two things it deliberately does not do: it never sums across currencies, because stores price in their own and a combined figure would be a number with no unit (converting needs dated exchange rates, a data source that does not exist here, and inventing one is worse than abstaining); and its window is UTC rather than any store's local day, because the query spans stores in different zones and picking one store's day would make the number right for it and wrong for the rest. **What a merchant is told about this is a disclosure question for the merchant agreement, not an engineering one** — recorded as an owner point, not assumed here.
