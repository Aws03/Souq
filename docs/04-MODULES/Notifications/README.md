...
- **`Email:Provider=Log` is an escape hatch for *absence*, not an override (clarified in M11).** A configured key wins over it: the selection is `Resend:ApiKey` → `Brevo:ApiKey` → `Gmail:AppPassword` → log, and `Email:Provider=Log` is consulted only in that last branch. So setting it on a machine that already has a key in user-secrets changes nothing, and mail goes to the real provider — which is exactly what happened while verifying M11 and cost a debugging detour. To force the log sender locally, remove the key; the setting will not do it.
...
| Log only | `ConsoleEmailService` + `ConsoleEmailOptions` | **only when no provider key is set at all** — automatically in Development and Testing, elsewhere with `Email:Provider=Log` | — |
...
| Integration (M14) | `NotificationTests` — the three recovery paths | **The purge** takes only rows processed before the cut-off, and leaves the unsent (deleting one means an email that never arrives), the dead (its error is the whole point of keeping it) and the recently processed. **The lease** protects a message while another process holds it, **and releases it when it expires** — the half that matters, since a process that died holding a lease must not freeze a message forever. **Two dispatchers racing** send the customer exactly one email, with the first held inside the provider while holding the lease so the window is genuinely open |
...
| Architecture (M14) | `NotificationDocumentationTests` | **The table above matches the code.** The existing documentation tests check that every name written down exists; nothing checked that everything that exists is written down, and omission is the direction that misleads a reader. A seventh outbox message type, or a fourth in-app kind, now fails a test until this document is updated |
