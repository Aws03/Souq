...
Catalog owns what a store sells and how it is presented: products with their per-language texts, their sellable variant (price, compare-at price, SKU), their media, and the category tree that organises them. It is its own module because product presentation changes at a different rate, under different rules, and by different people than stock levels (Inventory) or commercial records (Ordering). Categories are inside Catalog rather than beside it because they exist only to organise products ([Modules.md](../Modules.md)).

## Responsibilities

- The `Product` aggregate: texts per language, slug, status lifecycle, brand, video URL, an ordered image gallery, and exactly one default `ProductVariant` carrying price, compare-at price and SKU.
...
- **Default variant** — the sellable unit. It carries the SKU and the price; `Product.Price` and `Product.Sku` are shortcuts to it. Today every product has exactly one.
...
| Admin edit model | `GetAdminProductQuery` | `GetAdminProductHandler` | `catalog.manage` | `GET /api/admin/products/{id}` |
...
| Module | How |
|---|---|
| Inventory | Implements `IVariantStockInitializer`; `InventoryRepository.GetForProductAsync` filters on `ProductVariant.IsDefault`; `InventoryQueries` joins `Products`, `ProductVariants`, translations, images and categories for the admin screens |
| Shopping | `PricingService` loads `Product` aggregates via `IProductRepository.GetManyAsync` (name, translations, primary image, price, `IsActive`, `DefaultVariant`); `AddBasketItemHandler` loads a product to resolve its default variant and check `IsActive`; the wishlist handlers in `src/Souq.Application/Features/Wishlist/WishlistUseCases.cs` check `IsActive`; `WishlistQueries` reads `Products` and `InventoryItems` |
| Ordering | Indirectly, through Shopping's `IPricing`; `OrderItem` rows carry a tenant-scoped FK to `Products` (`OrderItemConfiguration`) |
...
| `ProductVariants` | `ProductVariantConfiguration` | yes | none | unique `(TenantId, Sku)` filtered `[Sku] IS NOT NULL`; unique `IX_ProductVariants_ProductId_Default` on `ProductId` filtered `[IsDefault] = 1` (exactly one default variant); alternate key `(TenantId, Id)` for tenant-scoped references from Inventory, baskets and orders; `Price`/`Currency` and `CompareAtPrice` as `decimal(19,4)` (`PersistenceConventions.MoneyColumnType`) |
...
| Frontend | `catalogText.test.js`, `productPayload.test.js`, `productQuery.test.js`, `categoryForm.test.js` | Language fallback and form mapping, the create/edit payload (including "never send stock on edit"), admin query keys, the category tree helpers |
...
1. **One variant per product.** `Product.DefaultVariant` is `_variants.Single(v => v.IsDefault)` and nothing creates a second variant. Attributes and the option matrix are DEFERRED ([ADR-0025](../../11-ADR/0025-catalog-model.md)).
2. **"Visible" means two different things.** `CatalogQueries.VisibleProducts()` and `WishlistQueries` require `Active` *and* an active category, while `Product.IsActive` (used by `PricingService` for `Sellable`, by `AddBasketItemHandler` and by the wishlist) only checks the status. A product in a hidden category therefore disappears from the storefront but can still be added to a basket and bought by id. No test covers this.
...
4. **Concurrency guards the `Products` row only** (see Domain model). Price, SKU, texts and images are last write wins.
...
- **Not buildable in the storefront today:** a variant picker. Every product sells through exactly one default variant (D-21) and `ProductDto` exposes no variant list, so a size/colour selector would have nothing to select. This is an API and model change, not a screen. The full analysis — every place the one-variant assumption lives, a proposed model, the order-line snapshot gap, a phased plan and the product decisions required (P-08) — is [ProductVariants.md](ProductVariants.md).
