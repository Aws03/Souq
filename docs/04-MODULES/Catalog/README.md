...
1. **No merchant can create a second variant yet.** Basket, pricing, checkout, order lines and stock administration handle several variants since V1 ([ADR-0039](../../11-ADR/0039-product-variants-order-identity.md)), but `Product.AddVariant` is internal and the option model (P-08a, decided) is V2 of [ProductVariants.md](ProductVariants.md). Every product still has exactly one variant, and the storefront projections still read the default one.
...
- **PLANNED** ([ADR-0039](../../11-ADR/0039-product-variants-order-identity.md), [ProductVariants.md](ProductVariants.md)): the variant option model and its admin (V2), then the storefront selection (V3), under decision P-08.
...
- **Not buildable in the storefront yet:** a variant picker. Nothing can define a product's options until V2, and `ProductDto` exposes no variant list until V3, so a size/colour selector would have nothing to select. The groundwork beneath it (V1) is done: the order records the variant, and basket, checkout and stock accept one. Status, decisions and the remaining plan: [ProductVariants.md](ProductVariants.md).
