...
| `ProductVariant` | entity in the Product aggregate | `src/Souq.Domain/Entities/ProductParts.cs` | Price > 0; compare-at price in the same currency and strictly higher than the price; SKU trimmed, upper-cased, `^[A-Za-z0-9][A-Za-z0-9._-]*$`, ≤ `SkuMaxLength`; `IsActive`, `CombinationKey` and `OptionValues`, changed only through `Product`. An optional `Cost` in the price's currency (C11), where absent means **unknown, not zero**, and a cost above the price is accepted because selling at a loss is a real commercial choice; it is a merchant secret and never appears on a shopper-facing type, which `MerchantCostSecrecyTests` fails the build over. Its constructor, `SetPricing`, `SetCost`, `SetActive`, `SetDefault` and `SetOptionValues` are `internal`, so only `Product` may change pricing, cost or activity. Never deleted: order lines, reservations, the ledger and basket lines reference it |
...
| Update a variant's price, compare-at, cost and SKU | `UpdateProductVariantCommand` | `UpdateProductVariantHandler` | `catalog.manage` | `PUT /api/admin/products/{id}/variants/{variantId}` |
...
- **Retention from day one.** TD-16 records that no table in this system has a retention policy, and that the ones holding personal data need a legal answer before they can get one. `SearchQueryLog` holds no personal data *by construction* — no customer id, no IP, no session or visitor token, and no column for one — so its retention was an engineering decision this phase could simply make: `Search:Log:RetentionDays` (90), applied by `SearchLogPurgeService` sending `PurgeSearchLogCommand` per store. `SearchAnalyticsTests` asserts the table's exact column set, so a later column that would invalidate that reasoning cannot be added quietly.
  ...
  **M15 corrected how that policy is applied.** As M13 shipped it, the command deleted a *single* batch per cycle — 5 000 rows every six hours, so 20 000 a day per store — while the write path is an anonymous, unrate-limited endpoint whose channel passes hundreds of rows a second. Ingest outran the purge by orders of magnitude, which means the ninety days this phase was proudest of were **not actually enforced** under any real load: the table would grow and rows past the cut-off would never be reached. The command now repeats its batches until one comes back short, with a per-cycle ceiling (`PurgeSearchLogHandler.MaxBatchesPerCycle`) so a table left alone for months cannot monopolise one sweep and starve the other stores; whatever remains is taken on the next cycle, and hitting the ceiling is logged rather than silent. The batch size itself is unchanged and still serves its original purpose — a small transaction on a table being written to at the same moment.
...
1. ~~Shoppers can't choose a variant yet.~~ **Resolved in V3 ([ADR-0041](../../11-ADR/0041-storefront-variant-selection.md)):** the storefront shows options and active variants, the shopper chooses explicitly, and prices read "From" the cheapest purchasable variant (BR-CAT-24 to BR-CAT-27). Variant **images** are still not built, so the gallery does not follow the selection.
...
- **PLANNED** ([ProductVariants.md](ProductVariants.md)): V4 — reporting per variant and the relabelled stock KPI. Variant images and manual variant ordering are not scheduled.
...
- **Built in V3** ([ADR-0041](../../11-ADR/0041-storefront-variant-selection.md)): `ProductDto` carries the options and active variants, the picker chooses one, and the storefront prices from what can be bought. Status and the remaining V4 work: [ProductVariants.md](ProductVariants.md). Status, decisions and the remaining plan: [ProductVariants.md](ProductVariants.md).
