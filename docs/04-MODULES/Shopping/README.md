...
- **Concurrency.** Neither table has a `rowversion`, and within a single basket the last write wins — a lost cart edit is not a financial record, and checkout re-validates everything ([ADR-0028](../../11-ADR/0028-basket-and-pricing-pipeline.md)). **That is not the whole story, and this bullet said it was until F-28.** Two other conflicts are real and both reached the shopper as a `409`, because the resolver writes: concurrent requests raced to delete the same guest basket after merging it (a delete matching no rows), and a customer with no basket yet had one created by *every* concurrent request, of which the filtered unique index on `(TenantId, CustomerId)` accepts exactly one. Those indexes do prevent two baskets per owner — by refusing the second, which is a conflict someone has to absorb. `BasketWriter` now absorbs both by retrying the resolve on committed state, so the guard stays and the shopper stops seeing it. The unique wishlist index prevents duplicates.
...
| 5 | Tax | An explicit zero. There is no tax model and no phase plans one; it is open product decision **P-06**. The stage keeps its fixed place in `PriceQuote` and `BasketDto`, so introducing tax changes neither of those contracts — but it is **not** a drop-in, and whoever answers P-06 should know why: Ordering has no tax slot at all. `Order.TotalAmount` is `(subtotal − discount) + shipping`, with no tax term, and `Place` snapshots it into `PlacedTotal`. The two agree today *only because tax is zero*; a non-zero tax in the pipeline alone would quote one figure and charge another. That divergence would be caught rather than shipped — `BasketTests` and `ShippingTests` both assert the order's total equals the quote's for the same basket — but closing P-06 means an `Order` column and a change to `TotalAmount`, not just this line |
...
- **Merge at sign-in.** A customer request that still carries a live guest token folds that basket into the customer's (creating one if needed) with `Basket.MergeFrom`, deletes the guest basket and clears the cookie. It happens on the first basket request, not at login, because the cookie is scoped to `/api/basket` and never reaches `/api/auth`. `CartContext` re-reads the basket whenever the signed-in user changes, which triggers it. `GetBasketHandler` therefore saves: **this GET can write** — and because it writes, it can conflict, which is what F-28 was. Anything entering this path concurrently (the checkout page quoting while the cart reloads, or simply a second tab) goes through `BasketWriter`.
