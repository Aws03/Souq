...
| Architecture | `ModuleAndContractRuleTests` | Only Catalog contracts inbound, only Ordering and Shopping outbound, no cycles; **no `StockMovement` is constructed anywhere in Infrastructure** (M7) — `Souq.Domain` opens its internals to `Souq.Infrastructure` for EF, so a ledger row *could* be built there without the matching `OnHand` change, breaking Σ ledger = on hand silently. No site does; the rule makes the next one fail the build. It scans `Newobj`, and was verified to fail against a deliberate violation |

**Not covered today:** the database check constraints firing (they are a last line of defence) and a suspended store's holds outliving their expiry.

The frontend inventory page **is** covered as of M7, having had nothing before it: `frontend/src/pages/admin/Inventory.test.jsx` (five tests — the row read cell by cell, a superseded page response not overwriting the displayed page, the previous page staying readable while the next loads, the low-stock count not re-fetched per page, and an adjustment refreshing table and banner together; the first three were each run against the pre-migration screen and observed to fail) and `frontend/e2e/admin-inventory.spec.js` on a live stack (the row's numbers, the movement drawer opened **from this screen**, and an adjustment that refreshes both and lands in the ledger with its reason). Phone width and touch targets for this screen are in `frontend/e2e/responsive.spec.js` under the `phone` project.
...
1. ~~The admin screen is still product-keyed.~~ **Resolved in V2 ([ADR-0040](../../11-ADR/0040-product-option-model.md)):** `frontend/src/pages/admin/Inventory.jsx` shows one labelled row per variant and uses the variant routes. The product-keyed routes remain for older clients and refuse a product with more than one stock row. The low-stock counts on the inventory page count variant rows, and the dashboard's stock figures do too until V4 relabels them. **M7 moved the screen onto the query layer** (part of [TD-25](../../12-ROADMAP/TechnicalDebt.md)): it had no guard against a superseded page response, so on a slow connection a response for a page the merchant had already left could paint its stock numbers under the wrong page label — on the one screen whose numbers are read and then acted on. The page is now the cache key, and an adjustment invalidates the shared `['inventory']` root so the table and the low-stock banner refresh together rather than one going stale under the other.
