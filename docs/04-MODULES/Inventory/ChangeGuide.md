...
- **Steps:** the interval and the batch size (`ExpireStaleCheckoutsCommand.Max`, default 50) are the safe knobs. For several application instances, add the distributed lock that Phase 23 planned rather than relying on idempotency alone — correctness holds today, but the duplicated gateway calls do not.
