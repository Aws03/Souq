...
- **Inspect:** `StorePaymentAccount`, `PaymentKeyRules`, `StorePaymentAccountEditor` and its published contract `IStorePaymentAccountEditor` (`src/Souq.Application/Features/Payments/StorePaymentAccounts.cs`, `src/Souq.Application/Features/Payments/Contracts/StorePaymentAccountContracts.cs`), the platform admin path `src/Souq.Application/Features/Platform/TenantPaymentAccounts.cs`, `PaymentGatewayRouter`, `AesGcmSecretProtector`, `SecretPurposes`, `StorePaymentsController`, `frontend/src/pages/admin/Payments.jsx`.
- **Rules to respect:** the editor is one implementation used by both the store path and the platform path, and the platform path runs **inside the target store's scope** so encryption, the write guard and the tenant filter all apply ([ADR-0024](../../11-ADR/0024-platform-administration.md)) — it reaches the editor only through `IStorePaymentAccountEditor` via `ITenantScopeRunner`, never the concrete class, because `TenantPaymentAccounts.cs` must stay in `Features/Platform` (its commands carry `TenantId`) while the editor lives in `Features/Payments`. Secrets are write-only. A connected-account model (Connect) replaces keys with an account id but must keep the same router seam — Application must not learn which model is in use.
- **Steps:**
...
- **Tests:** `tests/Souq.Application.Tests/Payments/StorePaymentAccountEditorTests.cs`; `tests/Souq.IntegrationTests/PaymentsAndRefundsTests.cs` (the store-account test covers encryption, audit and the platform path); add a routing test for the new model — note that no test names the router today.
