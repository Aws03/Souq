...
| `IUserRepository`, `IRefreshTokenRepository` | `src/Souq.Domain/Interfaces/IUserRepository.cs` | This module; also Notifications (see Dependencies). **Customers stopped using them in M9** — it goes through `IAccountLifecycle` below |
| `IAccountProfiles` | `src/Souq.Application/Features/Auth/Contracts/AccountContracts.cs` | This module (`RegisterHandler`, `AuthSessionIssuer`, `GetCurrentUserHandler`). **Declared here, implemented by Customers** (`CustomerAccountProfiles`) — a dependency inversion, the same shape as Catalog's `IVariantStockInitializer`. It is what removed every reference to `Customer` and `ICustomerRepository` from this module |
| `IAccountLifecycle` | `src/Souq.Application/Features/Auth/Contracts/AccountContracts.cs` | Customers (profile rename, erase-my-account's password check, customer erasure). Declared **and** implemented here (`AccountLifecycle`), so verifying a password, renaming an account, erasing it and revoking its sessions all happen inside the module that owns them. Before M9 another module did those things to `User` and `RefreshToken` directly |
