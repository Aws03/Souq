...
- **Testing:** fakes for driven ports (e.g. `FakePaymentService`). Driving adapters stay thin.
...
- **Now?** ✅ As a **principle**, not as an extra folder structure.
  - Souq already has ports: `IPaymentService`, `IEmailService`, `IFileStorage`, `IPasswordHasher`, `IJwtTokenGenerator`.
