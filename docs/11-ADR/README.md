...
| [0057](0057-cross-instance-coordination.md) | **One instance sweeps at a time**, through a `DistributedLeases` table claimed by a single conditional `UPDATE` — the outbox's mechanism generalised. Leases expire, and expiry *is* the recovery path; a lease per job rather than one for all background work; the holder renews between stores and **stops** when it loses it. `sp_getapplock` rejected as raw SQL, Redis as a recorded non-goal | **Implemented by `C4` (2026-09-22)** for the lock half, which is what `C6` was waiting on. Mutation-checked: removing the claim predicate fails four of its five tests |
