...
of it existed; `0053` and `0054` were implemented by `C1` and `C2`, `0055` records the tax capability the owner
decided on 2026-09-21 and is now implemented in full, and `0056` was implemented by `C5` the day it was written. The architecture they belong to is
...
| [0055](0055-tax-as-a-configurable-capability.md) | Tax is a **configurable, jurisdiction-aware capability**, never a rule in code: a platform-maintained jurisdiction profile, versioned and frozen on publish, that any store may select; inclusive/exclusive is a property of the version rather than of the platform; an immutable snapshot on every order and invoice, so history is exact; and **tax is collected only under a version a named professional marked verified** — engineering never sets that state, so a researched value cannot reach a shopper. Adds a fifteenth module, `Tax` | **Implemented (2026-09-22)**: the profile, its versions, the verification workflow, a store's selection, the calculator and the snapshot frozen onto orders — and, with `C5`, onto Souq's own invoices through the same gate ([0056](0056-platform-invoices-and-manual-collection.md)). Records the owner's answer to `P-06`, which reframed the question |
| [0056](0056-platform-invoices-and-manual-collection.md) | **Souq's own invoices**: one platform-wide number series per document kind allocated inside the issuing transaction, an issued invoice frozen and corrected only by a separate *CreditNote*, collection recorded as a human act with no payment provider in the loop, and *BillableEvent*/*BillingPeriod* as a ledger that prices nothing by itself. **The invoicing currency is configuration that fails closed**, because no currency literal may exist in `src/` — so `C-15`'s answer is entered once by the operator, not compiled in | **Implemented by `C5` (2026-09-22)**. Executes the owner's answers to `C-15` and `D-13`. Dunning (`C6`), automated card collection (`C-01`) and tier values (`C-12`) are named as deliberately out of scope |
| [0057](0057-cross-instance-coordination.md) | **One instance sweeps at a time, and every instance learns of an invalidation within seconds.** A `DistributedLeases` table claimed by a single conditional `UPDATE` — the outbox's mechanism generalised — where expiry *is* the recovery path and each job has its own lease; plus a `CacheSignals` generation counter that makes the two caches' existing local generations **shared**, polled every 5 s. `sp_getapplock` rejected as raw SQL, Redis as a recorded non-goal | **Implemented by `C4` (2026-09-22)**, both halves. The lock is what `C6` was waiting on, and it is mutation-checked: removing the claim predicate fails four of its five tests. Also corrects §4.18's classification of the rate limiter — it is a counter to be shared, not a cache to be invalidated |
...
- ADRs are numbered sequentially and never renumbered. The next one is **0058** (0047–0052 are the commercial-platform set, written 2026-09-20; 0053 was added by C1 and 0054 by C2 as each implemented them; 0055 records the owner's `P-06` answer of 2026-09-21, 0056 was written and implemented by C5 on 2026-09-22, and 0057 by C4 the same day; before them this line said 0047, and before 2026-09-20 it still said 0043 while 0043–0046 already existed — a record written from a stale line collides silently, so update it in the same commit that adds a record).
