...
- **Scope, and what happened to each.**
  ...
  - Product detail with gallery, variants, and reviews — **partly**. Gallery, reviews, breadcrumbs, a quantity stepper, slug URLs and schema.org data are done. **Variants are not yet:** no merchant can define options, and `ProductDto` exposes no variant list, so a picker would have nothing to pick. **Owner decision P-08 is taken** (2026-09-17: up to 3 structured options, 20 values and 100 variants; "From" the lowest purchasable price; an explicit choice with sold-out values disabled). **The groundwork (V1) is built** ([ADR-0039](../11-ADR/0039-product-variants-order-identity.md)): order lines record the variant with SKU and label snapshots, existing lines are linked to their exact variant, and basket, pricing, checkout and stock administration carry the variant. **Remaining:** the option model and admin (V2), then the storefront selection (V3) — [ProductVariants.md](../04-MODULES/Catalog/ProductVariants.md).
...
| P-08 | Product variants | ✅ **Decided 2026-09-17** ([ADR-0039](../11-ADR/0039-product-variants-order-identity.md)): structured options (≤3 options, ≤20 values each, ≤100 variants); "From" the lowest purchasable price in lists; an explicit choice with sold-out values shown disabled. V1 groundwork built; V2 (options and admin) and V3 (storefront) remain — [ProductVariants.md](../04-MODULES/Catalog/ProductVariants.md) | 16 |
