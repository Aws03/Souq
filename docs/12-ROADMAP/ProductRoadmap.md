...
### Phase 16: Storefront 🟡 *(was 15)* — delivered except one scope item
- **Scope, and what happened to each.**
  ...
  - Product detail with gallery, variants, and reviews — **partly**. Gallery, reviews, breadcrumbs, a quantity stepper, slug URLs and schema.org data are done. **Variants can't be chosen by shoppers yet:** `ProductDto` exposes no option or variant list, so there is no picker. **Owner decision P-08 is taken** (2026-09-17: up to 3 structured options, 20 values and 100 variants; "From" the lowest purchasable price; an explicit choice with sold-out values disabled). **The groundwork (V1) is built** ([ADR-0039](../11-ADR/0039-product-variants-order-identity.md)): order lines record the variant with SKU and label snapshots, existing lines are linked to their exact variant, and basket, pricing, checkout and stock administration carry the variant. **The option model and merchant admin (V2) are built** ([ADR-0040](../11-ADR/0040-product-option-model.md)): merchants define up to 3 options in Arabic and English, manage variants as combinations with their own price, SKU, stock and status, and inventory works per variant; until V3 the storefront shows only products with one active variant. **Remaining:** the storefront selection (V3), after two presentation confirmations — [ProductVariants.md](../04-MODULES/Catalog/ProductVariants.md).
...
| P-08 | Product variants | ✅ **Decided 2026-09-17** ([ADR-0039](../11-ADR/0039-product-variants-order-identity.md)): structured options (≤3 options, ≤20 values each, ≤100 variants); "From" the lowest purchasable price in lists; an explicit choice with sold-out values shown disabled. V1 groundwork and V2 (options and admin, [ADR-0040](../11-ADR/0040-product-option-model.md)) built; V3 (storefront) remains — [ProductVariants.md](../04-MODULES/Catalog/ProductVariants.md) | 16 |
