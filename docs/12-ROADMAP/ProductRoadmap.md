...
### Phase 17: Tenant admin dashboard 🟡 *(was 16)* — exit criterion met, one scope item open
- **Scope.**
  ...
  - Confirmation dialogs and server-paged data tables. **Partly.** The lists that grow — products, orders, customers, inventory, reviews, team — are server-paged. Destructive actions confirm through the browser's native `window.confirm`: it works with a keyboard and a screen reader, but it is unstyled and cannot carry the store's identity. **Remaining.**
...
### Phase 18: Platform owner dashboard 🟡 *(was 17)* — exit criterion met; preview, accounts, settings and audit screens open
- **Scope.**
  ...
  - Storefront preview using a preview token. **Remaining.** No preview token exists on the server; the settings editor's in-frame preview is not a substitute for seeing the real storefront closed.
  - ~~Platform statistics~~ **delivered**: `frontend/src/app/PlatformLayout.jsx` is the first real consumer of `GET /api/platform/stats`, and it states the counting limitations on screen rather than letting a reader assume that a count of stores excludes archived ones. Platform users and global settings **remain**.
  - An audit-log viewer. **Remaining**; `GET /api/platform/audit` exists.
