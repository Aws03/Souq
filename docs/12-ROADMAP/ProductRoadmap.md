...
- **Scope, and what happened to each.**
  ...
  - Product detail with gallery, variants, and reviews — **partly**. Gallery, reviews, breadcrumbs, a quantity stepper, slug URLs and schema.org data are done. **Variants are not, and cannot be from the storefront:** every product sells through one default variant (D-21) and `ProductDto` exposes no variant list, so a picker would have nothing to pick. It is a model and API change. **Analysed** in [ProductVariants.md](../04-MODULES/Catalog/ProductVariants.md): groundwork that needs no decision (recording the variant on order lines, carrying it through basket and checkout, variant-keyed inventory administration), and the customer-visible feature, which waits on owner decision P-08.
...
| **P-08** | Product variants | ⚠️ **Owner decision required** for Phase 16's remaining scope:<br>• (a) how variants differ: structured options vs free-form labels;<br>• (b) what price a multi-variant product shows in lists, filters, sorting and the on-sale badge;<br>• (c) selection and availability: preselect or require a choice; show or hide sold-out combinations.<br>Recommendation: up to three structured options; "from" the lowest sellable price; an explicit choice, with sold-out values shown disabled. Engineering groundwork needs no decision. Brief: [ProductVariants.md](../04-MODULES/Catalog/ProductVariants.md) | 16 |
