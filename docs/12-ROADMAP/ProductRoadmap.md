...
> **Status:** Phase 0 ✅ · Target architecture ✅ documented · Phase 1A ✅ (merged to `main`) · Phase 1B ✅ on branch `phase/1b-production-foundations` (awaiting review) · **Autonomous run on `phase/2-15-multitenant-platform`** (branched from the 1B tip): Phase 2 ✅ · Phase 3 ✅ · Phase 4 ✅ · Phase 5 ✅ · Phase 6 ✅ · Phase 7 ✅ · Phase 8 ✅ · Phase 9 ✅ · Phase 10 ✅ · Phase 11 ✅ · Phase 12 ✅ · Phase 13 ✅ · Phase 14 ✅ · Phase 15 ✅. An **engineering-knowledge and handoff pass** then ran on branch `phase/16-engineering-knowledge-and-handoff` (documentation system, module and boundary documentation, generated inventories, documentation tests) — it is not a roadmap phase, and the roadmap's Phase 16 (Storefront) has not started. A second engineering mission, **production hardening**, then ran on branch `phase/17-production-hardening` and **completed at commit `6a520b7`** (23 commits from the Phase 16 tip `d35c666`): payment-intent state machine, tenant-isolation, transaction, concurrency, idempotency and API-contract audits, and the release-readiness triage in [ReleaseReadiness.md](../09-OPERATIONS/ReleaseReadiness.md). It is **not** a roadmap phase either, and the roadmap's Phase 17 (Tenant admin dashboard) had not started at that point. **Phase 16 (Storefront) was then delivered** on `phase/17-production-hardening`. A further engineering mission, **storefront experience and management intelligence**, then ran on the same branch; it is not a roadmap phase either, but it delivered the KPI half of **Phase 17** and the statistics slice of **Phase 18**, so both are now 🔶 rather than ⏳ — recorded against those phases, not renumbered. **Phase 17's remaining screens (store settings and team) were then delivered** on the same branch, which meets its exit criterion; it stays 🟡 for one scope item (styled confirmation dialogs). **Phase 18's store list and provisioning wizard were then delivered** on the same branch, which meets Phase 18's exit criterion. **The remaining back-office scope was then completed** on the same branch: styled confirmations in the store admin close **Phase 17 ✅**; platform accounts and the activity log leave **Phase 18 🟡** only for two owner decisions — storefront preview (D-22) and platform-wide settings (P-07). An **engineering knowledge and learning system** pass then ran on the same branch (not a roadmap phase): a numbered reading path, request and code-reading guides, the invariants and "why" indexes, a project map, a handoff checklist, and an audit that corrected stale documents against the code. **Product variants** were then built in three phases on the same branch (V1 groundwork, V2 the option model and merchant admin, V3 the shopper's selection), which **closes Phase 16 ✅**; V4 (reporting per variant) remains.
> **Companion document:** [ArchitectureAssessment.md](../archive/ArchitectureAssessment.md) covers the current state, the problem register (IDs such as `B1` and `C2`), the target architecture, and the full reasoning behind every decision (`D-xx`). **The remaining execution — Phases 19–23 below, plus the search engine — has a durable, self-recovering execution contract:** [SouqMasterPlan.md](SouqMasterPlan.md), twenty engineering-mission phases (`M1`–`M20`, taking no roadmap number per §5's convention) with a machine-readable status block a fresh Claude session reads to continue an autonomous `souq continue` run with no prior conversation.
> **Last updated:** 2026-09-17 (added the SouqMasterPlan pointer)
...
**Engineering missions are separate and carry no roadmap number.** Hardening, release-readiness and knowledge work run on their own branches and close against the registers, not against this plan. Several have run so far — `phase/16-engineering-knowledge-and-handoff` (the documentation system), production hardening (completed at `6a520b7`), **operational readiness** (on the same branch, completed at `1bd8ace`), enforcement and monitoring, and **storefront experience and management intelligence**. None of them took a roadmap number. Four advanced the roadmap not at all; the last delivered slices of Phases 17 and 18 and those slices are recorded **inside** those phases, which stay open for the rest. The number in such a branch name is a branch sequence, not a phase. When an engineering mission needs a name, give it one ("production hardening", "operational readiness") rather than the next free roadmap number.
...
### Phase 16: Storefront ✅ *(was 15)* — closed by the variant picker (V3)
- **Scope, and what happened to each.**
  - A home page with tenant-configurable sections — **done**. The hero is the store's own name, description and image; it used to be one store's marketing copy shown to every tenant. A merchant-editable banner is a platform capability that does not exist (TD-42).
  - Catalog and search — **done**; search is URL state, Offers filters on the server's `onSale`.
  - Product detail with gallery, variants, and reviews — **done**. Gallery, reviews, breadcrumbs, a quantity stepper, slug URLs and schema.org data, and since V3 the variant picker: options and active variants come from the server, the choice is explicit, sold-out values are disabled, and the price reads "From" the cheapest purchasable variant. **Owner decision P-08 is taken** (2026-09-17: up to 3 structured options, 20 values and 100 variants; "From" the lowest purchasable price; an explicit choice with sold-out values disabled). **The groundwork (V1) is built** ([ADR-0039](../11-ADR/0039-product-variants-order-identity.md)): order lines record the variant with SKU and label snapshots, existing lines are linked to their exact variant, and basket, pricing, checkout and stock administration carry the variant. **V2 (the option model and merchant admin, [ADR-0040](../11-ADR/0040-product-option-model.md)) and V3 (the shopper's selection, [ADR-0041](../11-ADR/0041-storefront-variant-selection.md)) are built:** merchants define up to 3 options in Arabic and English and manage variants as combinations with their own price, SKU, stock and status; shoppers choose a variant explicitly, see its price and availability, and its label follows the line to the basket, the order and its email. **Remaining:** V4 — reporting per variant and the relabelled stock KPI — [ProductVariants.md](../04-MODULES/Catalog/ProductVariants.md).
  - Cart — **done**, a page at `/cart` beside the drawer.
  - Checkout: addresses, shipping, payment — **done**, and the invented "3–5 business days" promise is replaced by the order's own estimate.
  - An `/account` area: orders, profile, addresses — **done**, behind one `AccountLayout` shell.
  - Wishlist and order tracking — **done**. Both pages were broken: `OrderTracking` was a zero-byte file and `/wishlist` threw on open.
  - Per-tenant SEO — **done** for what a client-rendered app can do: per-page title, description, canonical URL, social tags, structured data and a robots.txt. **A crawler that runs no JavaScript still sees none of the meta tags**; that needs server rendering, and it is written down rather than implied.
  - WCAG 2.1 AA target, mobile-first — **partly**. axe runs over eight storefront surfaces on every test run and the dialogs became real dialogs (Escape, focus, scroll lock). Colour contrast and real focus order need a browser and were not measured.
- **Exit criteria: met.** The seventeen critical journeys were executed in a browser against a live stack — SQL Server, the API, two provisioned stores on different hosts — and pass. The second store is checked for its own identity, its own currency, its own catalog, and for seeing nothing of the first.
- **What this phase did not do, deliberately:** the feature-folder migration and the per-feature API modules (TD-24). Recorded with the measurement behind the decision — the admin half of the API client is 780 bytes gzipped, so the split buys structure, not speed.
- **Docs.** `FrontendArchitecture.md` §5–6, [ADR-0038](../11-ADR/0038-query-layer-adopted-and-type-checking.md).

### Phase 17: Tenant admin dashboard ✅ *(was 16)*
- **Scope.**
  - ~~KPIs: orders, revenue, average order value, low stock, recent activity.~~ **Delivered** by the storefront-experience mission (see the log entry for 2026-09-15). The store dashboard is one reporting request behind `store.reports.view`, with net revenue, counted orders, average order value, new customers, a daily trend, orders by status, best sellers, category performance, stock and customers — plus a second screen, the business overview, for an owner or investor rather than an operator. What every metric means, and the two that are deliberately absent, are in [Dashboards.md](../04-MODULES/Reporting/Dashboards.md).
  - Back-office screens for everything built in Phases 5–14. **Delivered.** Checked mechanically rather than by recollection: every store endpoint behind a permission in [Endpoints.md](../05-API/Endpoints.md) now has a caller in `frontend/src/api/client.js`.
  - Store settings, staff, and permissions. **Delivered** (log entry 2026-09-17): `/admin/settings` edits everything the store owns after handover, with a live preview in both modes and the Domain's rules checked before saving from limits the server publishes (`GET /api/admin/store/settings/options`); `/admin/staff` lists, invites, re-invites, enables and disables the team. *Permissions* are the two built-in store roles, explained on the invitation form; custom per-tenant roles remain a post-launch item (§11).
  - ~~Confirmation dialogs and server-paged data tables.~~ **Delivered** (log entry 2026-09-17, back-office completion). The lists that grow — products, orders, customers, inventory, reviews, team — are server-paged. Every destructive action confirms in `ConfirmDialog`, in the store's identity, with the server's refusal read inside the dialog; no screen uses `window.confirm`.
- **Exit criteria.** A tenant admin can run a store without ever using Swagger. **Met** for everything the store owns. What a store still cannot do from its own admin is, by design, the platform's: currency, domains and modules ([WhiteLabel.md](../08-FRONTEND/WhiteLabel.md) §2).
- **Verified in a browser** against a live stack: a saved announcement reaches an anonymous visitor; an unreadable palette never sends a request; the preview derives dark tokens inside its own frame; a disguised SVG logo is refused; invite then disable follows the server; axe finds nothing on either screen; both screens hold at phone width in both directions (`frontend/e2e/store-administration.spec.js`, `frontend/e2e/responsive.spec.js`).
- **Why it is not renumbered.** The dashboard was built because the experience mission needed it, not as a phase of its own. It is recorded against Phase 17 rather than given a new number, and the phase stays open for the work it still contains.

### Phase 18: Platform owner dashboard 🟡 *(was 17)* — exit criterion met; storefront preview and platform settings wait on owner decisions
- **Scope.**
  - ~~Tenant list and tenant health~~ **delivered** (log entry 2026-09-17): `/platform/stores`, server-paged, with each store's status, primary domain and administrator state. "Health" is handover readiness — a domain, an administrator who accepted — not a store's sales: no store's commercial figures reach the platform list, and the overview shows totals only.
  - ~~A **provisioning wizard**: identity → branding → domains → modules → admin → activate~~ **delivered**: `/platform/stores/new` then `/platform/stores/:id/setup/:step`. Branding is the store settings editor itself. Every step saves on its own and the store is born `Provisioning`, so an abandoned wizard leaves a closed store that the list resumes, never a half-open one. Readiness is reported and never blocks: the server does not require a domain or an administrator to activate, and the UI invents no rule it doesn't. A store page (`/platform/stores/:id`) holds the same sections afterwards, with lifecycle actions behind a confirmation — archiving requires typing the slug.
  - Storefront preview using a preview token. **Decision required (D-22)**, not built. A preview token is a new credential that lets requests past the store-status gate on another host, and who may hold one, for which states, doing what, for how long and carried how are security decisions the repository records no rule for ([AGENTS.md](../../AGENTS.md) §9). The options and a recommendation are in [StorefrontPreview.md](../04-MODULES/Platform/StorefrontPreview.md). The settings editor's in-frame preview is not a substitute for seeing the real storefront closed.
  - ~~Platform statistics~~ **delivered**: `frontend/src/app/PlatformLayout.jsx` is the first real consumer of `GET /api/platform/stats`, and it states the counting limitations on screen rather than letting a reader assume that a count of stores excludes archived ones. ~~Platform users~~ **delivered**: `/platform/accounts` invites an administrator or owner, resends, enables and disables — the operations the server offers, nothing more. Global settings: **decision required (P-07)** — `platform.settings.manage` exists, but no platform-wide setting is defined, and the screen would have nothing true to edit.
...
| **D-22** | Storefront preview access | ⚠️ **Owner decision required** before Phase 18 can close: who may preview a closed store, which states, read-only or not, lifetime and revocation, and how the credential crosses from the platform host to the store host. Recommendation: platform accounts only; `Provisioning` and `Suspended`; read-only; a revocable server-side grant; a one-time code in the URL fragment exchanged for an `HttpOnly` cookie on the store host; `no-store` and `noindex`. Brief: [StorefrontPreview.md](../04-MODULES/Platform/StorefrontPreview.md) | 18 |
| **P-07** | Platform-wide settings | ⚠️ **Owner decision required.** `platform.settings.manage` is granted to the platform owner, but no endpoint needs it and no setting is defined. Decide what is platform-wide rather than per store — for example, the platform's name in the console and in platform emails (a constant today), defaults for a new store, or a support contact — before a screen is built | 18 |
| P-08 | Product variants | ✅ **Decided 2026-09-17** ([ADR-0039](../11-ADR/0039-product-variants-order-identity.md)): structured options (≤3 options, ≤20 values each, ≤100 variants); "From" the lowest purchasable price in lists; an explicit choice with sold-out values shown disabled. V1, V2 ([ADR-0040](../11-ADR/0040-product-option-model.md)) and V3 ([ADR-0041](../11-ADR/0041-storefront-variant-selection.md)) built; V4 (reporting) remains — [ProductVariants.md](../04-MODULES/Catalog/ProductVariants.md) | 16 |
| **F-8** | Duplicate checkout submission | ⚠️ **Owner decision required:** replay the original order (`200`) or refuse the duplicate (`409`). Today two orders are created, with no double charge, and the duplicate expires (`CheckoutIdempotencyTests`). The implementation design is written; only the behaviour is undecided: [OwnerDecisions.md](../09-OPERATIONS/OwnerDecisions.md) | before idempotent checkout is built |
| **R-03** | Refunds on `orders.manage` alone | ⚠️ **Owner decision required:** cancelling a paid order issues a full refund, so a staff role that manages orders but not payments can move money. Keep it, or require `store.payments.manage` for a refunding cancellation. Evidence and options: [OwnerDecisions.md](../09-OPERATIONS/OwnerDecisions.md) | before the first paying customer |
...
| 2026-09-15 | **Operational readiness** engineering mission completed on `phase/17-production-hardening` — not a roadmap phase and it advanced none. Delivered: health checks (liveness/readiness, container probe without `curl`); a rehearsed backup and restore with the source stack destroyed first; separate runtime and migration database identities with **measured** permissions; security headers, HSTS and the forwarded-scheme fix that was blocking HSTS entirely; a CI pipeline (four jobs, audits, secret scan); a production configuration audit that closed a published JWT signing key in `.env.example` and a development CORS origin live in production; seed-safety proof on a fresh production-shaped database plus warnings about the unadopted default store; a migration-safety ratchet and a measured demonstration that rollback loses data; token leaks closed in the proxy access log; a 31-check smoke test; F-17 measured (11×→22×, index and rewrite both rejected on evidence); **F-7 fixed** (a store can no longer be left with no administrator); F-8 measured and fully designed, awaiting one owner decision; the frontend tooling advisories closed by a measured upgrade. Registers reconciled in full and a final release assessment produced. Zero NuGet dependencies changed. |
| 2026-09-15 | **Enforcement & monitoring** engineering mission completed on `phase/17-production-hardening` — again **not a roadmap phase**, and it advanced none. Converted operational mechanisms into enforced controls: an untrusted-proxy misconfiguration (the silent cause of missing HSTS, `http://` email links and a shared rate-limit bucket) is now detected at runtime; the database identity is read **from the database** at startup and reported when it can change the schema; backup freshness, integrity, completeness and drill age are checkable by one command, with the rehearsal writing its own evidence; the SPA's report-only CSP can no longer drift because a new external origin fails the build; the cookie attributes the entire CSRF defence rests on are asserted for the first time; `release-gate.sh` and `audit-config.sh` make the release checklist executable, counting anything unchecked as unchecked. Found and fixed a literal password in a tracked script that CI's secret scan would have failed on, and a bash-4 construct that breaks on the macOS system shell. Health contract drilled live (database down → 503, liveness 200, self-heal in ~11s, **zero restarts**). Measured: SQL Server holds 1.085 GiB idle, so the documented test-memory requirement rose from ~2 GB to 3 GiB. Zero NuGet dependencies changed. Product roadmap numbering untouched; next capability remains **Phase 16 — Storefront rebuild**. |
| 2026-09-15 | **Phase 16 (Storefront) delivered**, 17 commits — the first roadmap phase since 15. Four screens that were broken in the repository and only failed in a browser: `MyOrders.jsx` and `OrderTracking.jsx` were **zero-byte files** committed empty in Phase 9, and `/offers` and `/wishlist` used `t` before it was defined. Added: an account shell, a cart page, product URLs by slug with breadcrumbs, a quantity stepper, schema.org data, a real 404, an error boundary, robots.txt, and dialogs that actually behave as dialogs. Removed: an invented 3–5 day delivery promise, one store's marketing copy on every storefront, six dead footer links, a `JO` country default, `ar-JO` dates and the Stripe field's hard-coded font (TD-27 closed). Adopted at the points ADR-0037 named: **TanStack Query** for storefront server state and **`checkJs` + JSDoc** type checking, both in CI, plus **ESLint** — which the frontend had never had, and which found a JSX parse error Vite compiled silently. Tests 110 → 263, with axe over eight surfaces and a source invariant that no routed module can be empty again. Browser validation against two live tenants found a session defect no unit test could see: a 429 on refresh signed the customer out (SEC-SESS-01 fixed; G-19 recorded for a decision). **Not done, deliberately:** the feature-folder migration (TD-24) and a variant picker, which needs an API that exposes variants. |
| 2026-09-15 | **Storefront experience & management intelligence** engineering mission on `phase/17-production-hardening` — *not* a new roadmap phase. It delivered the **KPI half of Phase 17** and the **statistics slice of Phase 18**, which are recorded against those phases above rather than renumbered; both stay 🔶 for the screens they still contain. Built: a store-scoped reporting contract (one request per dashboard, all aggregation in SQL, a closed period key so the browser cannot ask for an arbitrary window); a **manager dashboard** ("what needs me now") and a separate **business overview** for an owner or investor ("is this healthy, is it growing, where is the risk") from the same response — two pages disagreeing about one number would mean one of them is lying; a platform overview that states its counting limitations on screen; hand-written SVG charts with table alternatives, chosen over a ~90 kB library against a ~135 kB first load; **dark mode** derived per store rather than overridden in CSS; an **opening experience** a store opts into, governed by five conditions; four product-card layouts; and the phone-width management UI. Profit, margin, conversion and forecasting are **declared absent** on both dashboards, because no cost price and no visit tracking exist — see [Dashboards.md](../04-MODULES/Reporting/Dashboards.md). Defects found only by opening the product in a browser: a branding field the API accepted with `204` and never stored (the settings document and the domain model are separate); a white flash on every load for a dark-mode visitor; a dark mode whose inverted panels — footer, hero, admin sidebar — turned pale with dark text, because their two tokens flip together; a featured section that collapsed to a strip beside a void; "1 orders", a category rendered as its URL slug, and Arabic names breaking adjacent punctuation in English sentences; a mobile tab bar truncating labels to three letters; and a log-out button that fell off a 720-pixel screen. Frontend tests 263 → 404 (47 → 60 files), backend 1,074 → 1,112, browser checks 24 → 47 across two viewports, first load 142.1 → 136.5 kB gzip in Arabic and 134.2 in English — it **fell** while the mission added dark mode, charts and two dashboards, because the locale bundles became per-language dynamic imports. Zero dependencies added. |
| 2026-09-17 | **Phase 17 screens: store settings and team**, on `phase/17-production-hardening`. The phase's exit criterion — a tenant admin runs the store without Swagger — is now met; it stays 🟡 for styled confirmation dialogs. Chosen over the Phase 18 provisioning wizard because every store needs it after handover, its API already existed, and the wizard's branding step is the same settings document: the editor built here is what that step will reuse. Backend: one read-only endpoint, `GET /api/admin/store/settings/options`, publishing the Domain's allowlists, limits and contrast thresholds, with a test that passes every offered option back through the Domain — so the frontend holds no copy of the rules. Frontend: `/admin/settings` (identity, branding uploads, colours with live readability checks, typeface, preset, mode, opening reveal, languages, time zone, contact, social, search text, announcement; currency read-only) with a preview that applies the storefront's own derived tokens in light and dark, and `/admin/staff`. Found only by running axe in a browser, and fixed at the source rather than on the new pages: every admin table's row-actions button had no accessible name; status badges computed their text against the page, not their own tint (green 4.20:1); the active sidebar link fell to 3.39:1. Backend tests 1,112 → 1,122; frontend 404 → 452; browser journeys 47 → 56, all passing when each file runs in its own minute (run back to back, the ten-a-minute sign-in limit refuses the later files' logins — the control working, not a defect). Zero dependencies added. |
| 2026-09-17 | **Phase 18: store list and provisioning wizard**, on `phase/17-production-hardening`. Exit criterion met; the phase stays 🟡 for storefront preview (needs a server-side preview token), platform accounts and settings screens, and the audit-log viewer. The backend already had the whole provisioning API and a test of its lifecycle; what it lacked for a UI was small and is recorded as such: `GET /api/platform/tenants/options` (identity limits, reserved slugs, modules, and the settings editor's own options object, compared byte for byte with the store's), administrator counts on the store list, and one rule — a platform host can never be a store domain (BR-TEN-27, `409 DomainReserved`), because uniqueness had been checked between stores only. Two boundaries were proven for the first time: a platform owner's token is refused on a store it provisioned, and a new store's administrator sees nothing of another store. Frontend: the store settings editor became a shared component with a data source, used unchanged by `/admin/settings` and by the wizard; shared store panels; `ConfirmDialog` with typed confirmation. Found only in a real browser and fixed where they live, not on the new pages: `FormField` never linked its label to the input on sign-in, registration, password reset and invitation acceptance; accent buttons and dark-mode text on cards failed contrast in every store's dark mode; and the platform host had no dark primary or status colours at all. Backend tests 1,122 → 1,126; frontend 452 → 498; browser journeys 56 → 66, all passing with each file in its own minute. First load +0.64 kB gzip, mostly the platform functions in the shared API client. Zero dependencies added. |
| 2026-09-17 | **Back-office completion: confirmations, platform accounts, activity log** (Phases 17–18 remaining scope), on `phase/17-production-hardening`. **Phase 17 ✅.** Phase 18 stays 🟡 for two owner decisions, not for engineering: **storefront preview (D-22)** — a preview token is a new credential that opens a closed store across hosts, and who holds one, for which states, doing what, for how long and carried how are stop-and-ask security decisions ([StorefrontPreview.md](../04-MODULES/Platform/StorefrontPreview.md) sets out options and a recommendation); and **platform-wide settings (P-07)** — `platform.settings.manage` exists but no setting is defined, so a screen would have nothing true to edit. Built: the last seven `window.confirm` prompts moved to `ConfirmDialog` through one hook, with the server's refusal read inside the dialog and a dialog over a drawer closing alone on Escape; `/platform/accounts` with exactly the operations the server offers (invite administrator or owner, resend, enable, disable — no role change, no deletion); `/platform/audit` on the server's filters and paging, filters in the URL applied by a button because reading the log writes a line, entry details with recorded metadata, IP and request id, ids and roles rather than fetched names, and a statement on screen of what the log never contains. Found while building it and fixed at the API boundary: **every timestamp read from the database was sent without a zone**, so browsers showed order times, last sign-ins and audit lines shifted by the reader's offset (three hours early in Amman); `UtcDateTimeJsonConverter` now writes `Z`. Found in a real browser: the success toast paired `--color-on-panel` text with the primary background. Backend tests 1,126 → 1,131; frontend 498 → 525; browser journeys 66 → 72, all passing with each file in its own minute. The two new screens are lazy chunks on the platform host (1.5 and 4.1 kB gzip). Zero dependencies added. |
| 2026-09-17 | **Engineering knowledge and learning system**, on `phase/17-production-hardening` — an engineering mission, *not* a roadmap phase; no phase status changed. The existing documentation was audited against the code first, in six areas, before anything was written; every finding was re-verified before it was applied. **Added** in `docs/00-START-HERE`: [LearningPath.md](../00-START-HERE/LearningPath.md) (the numbered path 00–18 with learning levels L0–L3 and a concept index), [RequestLifecycle.md](../00-START-HERE/RequestLifecycle.md) (one request through every layer), [HowToReadTheCode.md](../00-START-HERE/HowToReadTheCode.md) (the tracing method with sign-in, the storefront catalogue and store suspension), [CriticalInvariants.md](../00-START-HERE/CriticalInvariants.md), [WhyItIsBuiltThisWay.md](../00-START-HERE/WhyItIsBuiltThisWay.md), [ProjectMap.md](../00-START-HERE/ProjectMap.md) and [HandoffChecklist.md](../00-START-HERE/HandoffChecklist.md); [HowToAddAFeature.md](../00-START-HERE/HowToAddAFeature.md) rewritten around a real feature (store shipping methods). **Corrected against the code**, across the architecture, module, API, database, security, frontend, operations and testing documents:<br>• statements that predated Phases 16–18, TanStack Query, CI and the security headers;<br>• a raw-SQL rule stated wrongly;<br>• a suspended store described as refusing sign-in;<br>• duplicated control and gap ids;<br>• P-05 described as "handled in both shapes" when the converter sends ×100 only;<br>• F-19 (UTC instants) still marked open;<br>• the five copies of the gate commands, which missed lint and type-check, now point to one canonical page. **New documentation tests:** every current document is reachable by links from the entry pages; the path's steps stay numbered 00–18; navigation pages carry a dated "last verified" line. Each test was shown to fail on a planted defect. No application behaviour changed; one stale setup comment in `frontend/playwright.config.js` was corrected. |
| 2026-09-17 | **Product variants V3 — the shopper's selection**, on `phase/17-production-hardening` ([ADR-0041](../11-ADR/0041-storefront-variant-selection.md)). The temporary V2 storefront gate is removed: options and active variants reach the product page, the choice is explicit with sold-out and impossible values disabled, lists price "From" the cheapest **purchasable** variant and filter, sort and structured data use the same number, and the label follows the line to the basket, the order and its email. The two open presentation questions were answered from the repository's lifecycle rules (deactivated variants hidden; a fully sold-out product stays listed). **Phase 16 ✅.** V4 (reporting per variant, relabelled stock KPI) remains. |
| 2026-09-17 | **[SouqMasterPlan.md](SouqMasterPlan.md) created** at checkpoint `d45bb53` on `phase/17-production-hardening` — no roadmap phase changed. A durable, repository-resident execution contract for the twenty engineering-mission phases (`M1`–`M20`) that carry Phases 19–23 above, plus first-class scope this roadmap's five-line tail did not break out on its own (principally a professional local, deterministic, multilingual search engine and search analytics — `M3`/`M13`). Defines the `souq continue` / `souq continue phases N-M` / `souq continue until launch` protocol, a strict per-phase completion procedure (discover → implement → test → run the real application → Docker verification → representative flows → fix defects → full release gate → docs → commit → push → verify remote → clean tree), explicit STOP conditions matching `AGENTS.md` §9 and every open item in [OwnerDecisions.md](../09-OPERATIONS/OwnerDecisions.md), and a machine-readable §0 status block so a fresh Claude session with no prior conversation can recover exactly where the work stands. No phase executed yet. |
