...
- **Scope, and what happened to each.**
  ...
  - Product detail with gallery, variants, and reviews — **partly**. Gallery, reviews, breadcrumbs, a quantity stepper, slug URLs and schema.org data are done. **Variants are not, and cannot be from the storefront:** every product sells through one default variant (D-21) and `ProductDto` exposes no variant list, so a picker would have nothing to pick. It is a model and API change.
