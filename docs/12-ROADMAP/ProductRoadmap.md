...
### Phase 18: Platform owner dashboard 🔶 *(was 17)* — the statistics slice delivered
- **Scope.**
  - Tenant list and tenant health. **Remaining.**
  - A **provisioning wizard**: identity → branding → domains → modules → admin → activate. **Remaining.**
  - Storefront preview using a preview token. **Remaining.**
  ...
  - An audit-log viewer. **Remaining.**
- **Exit criteria.** The platform owner can provision a new client store end-to-end in the UI. **Not met.**
