...
| Basket | `/api/basket` (Phase 8): `GET`, `GET /quote?couponCode=`, `POST /items`, `PUT` and `DELETE /items/{productId}`, `DELETE` | required | anonymous (guest cookie) or customer — only the caller's own basket |
