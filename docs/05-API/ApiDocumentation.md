...
- **Search:** `keyword`, matched on the stored **normalized** form of catalog text since M3 ([ADR-0042](../11-ADR/0042-local-search-engine.md)). Every word in the keyword must match, in a product name, its description or its category name, in any language. `sortBy=Relevance` ranks server-side; it falls back to `Newest` when there is no keyword.
  ...
  - **`/api/admin/search-synonyms`** (`catalog.manage`): `GET` lists the store's vocabulary, `POST` adds a directed one-word pair `{ "culture", "term", "expansion" }`, `PUT /{id}` replaces it, `DELETE /{id}` removes it. Codes: `SearchSynonymExists` (409, measured on the **normalized** form, so "مكنسة" and "مكنسه" are the same pair), `SearchSynonymLimitReached` (422), `InvalidSearchSynonym` (422 — one word per side, a supported language, and not a word mapped to itself).
...
- **Five lists are deliberately not paged, and two exports are unbounded.** Corrected in Phase 17 — this page previously claimed every list was paged, which was never true:
  ...
  - **bounded without paging (F-21, closed 2026-09-21):** `/api/categories`, `/api/admin/categories` and `/api/admin/shipping-methods` are returned whole, because each is consumed as a single unit — half a category tree is not a tree, and a partial shipping list hides an option from a buyer. They now carry a server ceiling of `PagingRules.MaxUnpagedItems` (1000), which no real store approaches; reaching it truncates **and logs a warning**, because a list that shortens silently turns a missing category into a bug hunted somewhere else entirely.
  - **the two `/export` endpoints stay complete, and are rate limited instead.** They return every order a customer ever placed with all of its lines plus every review, so they grow with the age of an account rather than with a requested page. Truncating was rejected outright: a data-subject export that omits data is not an export, and the endpoint exists to satisfy that right. What is capped is how often it may be asked for — `RateLimitPolicies.Export`, six per hour, on the customer's own endpoint and on the administrator's, because a permission says *who* may export and not *how often*.
- **Implementation (1B):**
