...
public sealed class LocalFileStorageTests : IDisposable
{
    ...
    private LocalFileStorage Storage(int tenantId = 7) =>
        new(Options.Create(new FileStorageOptions { RootPath = _root, PublicBasePath = "/uploads" }),
            SouqApiFactory.ContextFor(new TenantInfo(tenantId, "store", "متجر", TenantStatus.Active, "JOD", "ar", "Asia/Amman", new HashSet<string>(StringComparer.Ordinal), new Dictionary<string, int>(StringComparer.Ordinal))));
