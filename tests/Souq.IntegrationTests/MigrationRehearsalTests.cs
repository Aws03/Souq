...
public class MigrationRehearsalTests
{
    ...
    private static TenantContext Context(int tenantId) => SouqApiFactory.ContextFor(
        new TenantInfo(tenantId, "marka", "Marka Demo", TenantStatus.Active, "JOD", "ar", "Asia/Amman", new HashSet<string>(StringComparer.Ordinal), new Dictionary<string, int>(StringComparer.Ordinal)));
