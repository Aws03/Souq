...
// شاشة جرد المخزون: كل المنتجات النشطة بمخزونها الحالي (الأقلّ أولاً)، ملوّنة
// حسب المستوى، مع درج يعرض سجلّ حركة المخزون لكل منتج (بيع/توريد/تصحيح).
export default function Inventory() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api.getInventory()
      .then((res) => { setItems(res); setError(null); })
...
// درج سجلّ حركة المخزون لمنتج واحد — يجلب الحركات عند الفتح (الأحدث أولاً).
function StockMovementDrawer({ product, onClose }) {
  ...
  useEffect(() => {
    let alive = true;
    api.getStockMovements(product.id)
      .then((res) => { if (alive) setMovements(res); })
