...
describe('DataTable — منطقة مُمرَّرة يصلها من لا يملك فأرة', () => {
  ...
  // ==========================================================================
  // هيكل التحميل صفوفُ <tr> حقيقية لا يميّزها عن صفوف البيانات شيء في DOM. فقارئ الشاشة
  // يعلن خمسة صفوف فارغة كأنّها نتيجة، ومن ينتظر الجدول يظنّه وصل — وهو ما وقع: رحلة الجرد
  // كانت تنتظر أوّل <tr> ثمّ تقرأ، فتقرأ الهيكل ولا تجد صفّها فتضغط "التالي" وتتخطّى الصفحة
  // التي كان فيها. قِيس على الحزمة: الصفحة الأولى خمسةُ صفوفٍ فارغة والصفّ فيها فعلاً.
  //
  // والقيمة تُكتب صراحةً في الحالتين ("false" لا حذف السمة): انتظارٌ على `aria-busy="false"`
  // يحتاج السمة موجودة، وسمةٌ محذوفة تعني «لا أدري» لا «انتهى».
  // ==========================================================================
  it('الجدول يعلن انشغاله أثناء التحميل، ويعلن انتهاءه صراحةً', () => {
    const { unmount } = table({ loading: true, rows: [] });
    expect(screen.getByRole('region', { name: 'الفريق' })).toHaveAttribute('aria-busy', 'true');
    unmount();

    table();
    expect(screen.getByRole('region', { name: 'الفريق' })).toHaveAttribute('aria-busy', 'false');
  });
});
