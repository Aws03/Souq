...
- **Every known defect is recorded, including the ones already fixed**, with how it was found and what the fix was — [ReleaseReadiness.md](docs/09-OPERATIONS/ReleaseReadiness.md). The most recent (F-33) is a good example of the standard: a concurrent basket read answered `500` on a CI runner, the assertion was instrumented rather than guessed at, the captured message named a SQL deadlock whose victim was a *read*, and the fix covers that whole class with a deterministic test behind it.
