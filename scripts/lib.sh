...
# طابع UTC ⇒ ثوانٍ منذ العهد. يقبل صيغتي المستودع معاً: `20260918T140501Z` (اسم مجموعة نسخ)
# و`2026-09-18T14:05:01Z` (ملف دليل تجربة الاستعادة).
#
# **ولا يستعمل python3.** كان `backup-verify.sh` يحسب العمر بمفسّر بايثون، وعلى مضيف بلا
# مفسّر كان يطبع "لا فحص عمر" ويخرج بنجاح — أي أنّ أهمّ إنذار في النسخ الاحتياطي («هل
# النسخة حديثة؟») كان يمرّ دائماً على خادم مُقتصَد، وهو بالضبط الفحص الذي يمرّ دائماً الذي
# يحذّر منه رأس ذلك الملف. و`date` موجود حيث وُجد shell، لكنّه يختلف: GNU يقرأ بـ `-d`
# وBSD/macOS بـ `-j -f`، فتُجرّب الصيغتان — كما يفعل `sha256_of` أعلاه مع shasum/sha256sum.
utc_epoch() {
    local stamp="${1//[-:]/}"
    [[ "$stamp" =~ ^([0-9]{4})([0-9]{2})([0-9]{2})T([0-9]{2})([0-9]{2})([0-9]{2})Z?$ ]] || return 1
    local iso="${BASH_REMATCH[1]}-${BASH_REMATCH[2]}-${BASH_REMATCH[3]} ${BASH_REMATCH[4]}:${BASH_REMATCH[5]}:${BASH_REMATCH[6]}"
    date -u -d "$iso" +%s 2>/dev/null && return 0
    date -u -j -f '%Y-%m-%d %H:%M:%S' "$iso" +%s 2>/dev/null && return 0
    return 1
}
