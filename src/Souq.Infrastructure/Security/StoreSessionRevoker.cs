...
internal sealed class StoreSessionRevoker : IStoreSessionRevoker
{
    ...
    public async Task<int> RevokeAllAsync(int tenantId, string reason, CancellationToken ct = default)
    {
        ...
        // وإسقاط الأختام المخزَّنة على هذه النسخة: بلا هذا تبقى جلسةُ متجرٍ أُرشف مقبولةً حتى 30
        // ثانية، و«الأرشفة تُبطل الجلسات» إمّا أن تكون صحيحة أو لا تُقال. الكلفة قراءةُ ختمٍ واحدة
        // لكل حساب نشط بعدها، والسبب نادر.
        _sessions.ForgetAll();
        return accounts;
    }
}
