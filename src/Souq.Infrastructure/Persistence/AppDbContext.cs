...
public class AppDbContext : DbContext, IUnitOfWork
{
    ...
    public override async Task<int> SaveChangesAsync(CancellationToken ct = default)
    {
        ...
        catch (Exception ex) when (IsDeadlock(ex))
        {
            ...
            //
            // ثمّ تبيّن أنّ هذا الموضع — وأخوه في `InTransactionAsync` — لا يكفيان: كلاهما مسارُ
            // **كتابة**، والجمود يقع على القراءة كذلك (F-33). فالقاعدة العامّة صارت في
            // `DeadlockTranslatingInterceptor`، ويبقى هذان لأنّهما يريان ما لا يراه: تدبيرَ أحداث
            // المجال (`Abandon`) عند الفشل.
            // ====================================================================
            raised.Abandon(this);
            throw new ConcurrencyConflictException(ex);
        }
