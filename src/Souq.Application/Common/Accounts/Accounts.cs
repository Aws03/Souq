...
public sealed class AccountStatusChanger
{
        ...
        int userId, bool active, Func<string, bool> managedRole, string lastStandingRole, CancellationToken ct)
    {
        ...
        //
        // نوقف الحساب ثم نعيد العدّ داخل المعاملة نفسها: من يخسر السباق يرى صفراً ويتراجع كلياً.
        //
        // ── F-29: وهذا الترتيب وحده لم يكن كافياً، لأن فرضيّته كانت خاطئة ──────────────
        // كان هذا التعليق يقول: "تحت READ COMMITTED القافل — وهو افتراضي SQL Server، **ولا يُفعّل
        // هذا المستودع RCSI في أي مكان**". الشطر الأخير غير صحيح، وقيس: **EF Core يُفعّل RCSI على
        // كل قاعدة يُنشئها هو**، فكل نشرٍ تركَ الهجرات تُنشئ قاعدته يعمل بها. وتحتها يقرأ العدّ
        // الثاني **لقطة** فلا ينتظر شيئاً، فيمرّ المتسابقان معاً ويُوقَف آخر مديرَين — بلا استثناء
        // ولا سجلّ ولا اختبار أحمر.
        //
        // فالمعاملة تُفتح الآن بـ SERIALIZABLE **حين يكون الثابت في خطر وحده** (`guarded`): هو
        // المستوى الوحيد الذي يمنع تغيّر المدى المعدود قبل الالتزام، مهما كان إعداد القاعدة.
        //
        // والاعتراض الذي كان مكتوباً هنا على SERIALIZABLE — "جمودٌ يُنهي أحدهما بخطأ خادم بدل رفض
        // عمل واضح" — **أجابته الشيفرة التي كُتبت بعده**: `InTransactionAsync` تترجم الجمود إلى
        // `ConcurrencyConflictException`، والمصيدة أدناه تُعيد العدّ وتردّ رفضاً تجارياً واضحاً.
        // أي أن آلة معالجة الجمود كانت موجودة أصلاً، ومبنيّةً لهذا السباق بعينه.
        //
        // ولماذا لا sp_getapplock: قفل مسمّى خاص بـ SQL Server يضيف مفهوماً جديداً لحلّ ما تحلّه
        // المعاملة. ولماذا لا شكل العدّاد في ADR-0049: ذاك يحرس **سقفاً** يُعَدّ صعوداً، وهذا
        // **أرضيّة** تُعَدّ نزولاً على صفوف يملكها آخرون — لا صفّ واحد يُقفَل هنا.
        try
        {
            await _uow.InTransactionAsync(async () =>
            {
            ...
            // العزل الأعلى للحالة المحروسة وحدها: إيقافُ حسابٍ عاديّ لا ثابت له عبر صفوف، ودفعُه
            // ثمنَ أقفال المدى بلا سبب يجعل كل إيقاف أبطأ وأقرب إلى الجمود بلا مقابل.
            guarded ? TransactionIsolation.Serializable : TransactionIsolation.Default, ct);
        }
