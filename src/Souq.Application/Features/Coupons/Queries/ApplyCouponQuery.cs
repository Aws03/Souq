...
// معاينة خصم كوبون قبل الدفع (بلا أي تعديل على شيء). لا تستدعيها الواجهة اليوم: صفحة الدفع
// تعيد تسعير السلة عبر /api/basket/quote، وهو المسار الذي يفحص وحدة العروض وحدّ الاستخدام
// لكل عميل — فهذه النقطة تقييم ثانٍ أضعف للكوبون نفسه (دَين موثَّق: TD-06، تُحذف أو يُعاد
// بناؤها فوق IPricing). CreateOrderHandler يعيد التحقّق مستقلاً عند الدفع الفعلي.
